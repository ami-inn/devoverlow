...
> {
  ...
  if (query) {
    // fliterquery is dynamic based on whether the user is searching by name or email, we can use $or operator to search in both fields, and $regex for partial matching, and $options: "i" for case-insensitive search
    filterQuery.$or = [
      { name: { $regex: query, $options: "i" } }, // case-insensitive search on name and email fields
      { email: { $regex: query, $options: "i" } }, // case-insensitive search on email field
    ];
  }
  ...
  switch (filter) {
    ...
    case "oldest":
      ...
      break;
    case "popular":
      sortCriteria = { reputation: -1 }; // reputation field should be indexed for better performance
      break;
